...
resource "aws_iam_group_membership" "engineers_members" {
  ...
  users = [
    for user in aws_iam_user.users : user.name if user.tags.Department == "Engineering" && contains(keys(user.tags), "JobTitle") && can(regex("Engineer|Developer", user.tags.JobTitle))
  ]
}